...
public://公開
	...
	void drawing_img(Filter *filter, Move *move, int x, int y, int f);//画像読み込み描画処理
	virtual void draw(Filter *filter, Move *move, int x, int y);     //画像描画処理,virtual オーバーライドされる関数
	~ImageRead();                                                   //デストラクタ
