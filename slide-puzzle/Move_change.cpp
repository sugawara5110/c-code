...
int Move::change(Dx9Init *dx, MSG *msg, ImageDraw *draw){ //ブロック交換関数宣言
	...
	while (cnt < prs->idx){//z方向飛び出し処理
		...
		draw->theta(1, 5);
		if (draw->draw_img(dx, this, 0, 0, 0) == -1) return -1;//画像描画
		dx->drawscreen();//描画
	}//z方向飛び出し処理終了
...
int Move::shuffle(Dx9Init *dx, MSG *msg, ImageDraw *draw){   //シャッフル関数宣言 
	...
	return change(dx, msg, draw);//change返り値をそのまま返す
}

int Move::recovery(Dx9Init *dx, MSG *msg, ImageDraw *draw){//元通り
